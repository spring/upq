    ...
    def readConfig(self):
            ...
            if section == "logging":
                ...
                for path in self.paths.keys():
                    if self.paths[path][:2] == "./":
                        ...
                        self.paths[path] = os.path.realpath(os.path.dirname(__file__)) \
                        +self.paths[path][1:]
                ...
                if config.getboolean(section, "enabled"):
                    ...
                    self.jobs[section.split()[1]]['concurrent'] = int(
                        self.jobs[section.split()[1]]['concurrent'])
                else:
                ...
                self.conf_log("Unknown section '%s' found in config file, " \
                         +"ignoring.", section)
        ...
        for job in self.jobs.keys():
            if not self.jobs[job].has_key("tasks"):
                ...
                self.jobs.pop(job)
                self.logger.error("Found job '%s' without tasks, ignoring it.",
                                  job)
                continue
            if not reduce((lambda x, y: x and y),
                ...
                # key in tasks
                self.jobs.pop(job)
                self.logger.error("Found job '%s' with an unknown task, " \
                                  +"ignoring it.", job)
                continue
